(function(global, $) {
   ...
   BandRatioClient.prototype.setup_binds = function() {
      ...
      $(this.eventObj).bind('band_ratio.dropped', this.onBandDrop.bind(this));

      // specific handler for when a band number is dropped on the design pane
      $(this.eventObj).bind('band_ratio.freq_dropped', this.onFreqDrop.bind(this));

      // simple handlers for run/show query buttons

      this.runQueryEL.bind('click', this.queryClick.bind(this));

      this.showQueryEL.bind('click', this.showQuery.bind(this));
   ...
   BandRatioClient.prototype.generateWCPS = function() {
      ...
      if (this.operations.length === 1) {
         ...
         this.operationQuery = this.operations[0].rootRender();
         query += 'return encode( (char) (255 / max';
   ...
   BandRatioClient.prototype.getMedian = function(freq, elem) {
      ...
      var min_query = this.min_query.replace(/FREQ/g, freq);
   ...
   BandRatioClient.prototype.getAvg = function(freq, elem) {
      ...
      if (this.testFreq(freq)) {
         ...
         $.ajax({
            ...
            this.errorDialog('Incorrect band given. Please enter just the number, do not include the "Band"');
         }
   ...
   BandRatioClient.prototype.getMinMax = function(minmax, frq, elem) {
      ...
      if (minmax === 'min') {
         query = this.min_query.replace(/FREQ/g, frq);
      }
      if (minmax === 'max') {
         query = this.max_query.replace(/FREQ/g, frq);
      }
   ...
   BandRatioClient.prototype.binDrop = function(ev, ui) {
      ...
      if (uiHelper.hasClass('holder')) {
         ...
         freq = prompt('please enter a band number');
         newClone = uiHelper.clone(true).removeClass('ui-draggable-dragging').css({
            posistion: 'relative',
            left: 0,
            top: 0,
